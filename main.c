...
#define I2C_SLAVE_ADDR_SWCH  0x74				// I2C address of switch (tca9548apwr)
#define I2C_SLAVE_ADDR_PLL   0x55				// I2C address of DSPLL  (Si570 - 570CAC000141DG)
#define I2C_SLAVE_ADDR_EROM	 0x51				// I2C address of EEPROM (M24C02)
#define SIZE_wr			17
#define SIZE_rd			8
...
void CalculateReg(u8 *RegBuffer, float Freq_new, XIicPs *I2C);
float Input();
...
/*!
 *  PLL operates only with the following frequency range:
 * 		10.000 MHz to 945.000 MHz
 * 		970.000 MHz to 1134.000 MHz
 * 		1212.500 MHz to 1417.500 MHz
 *
 * 	Principle of operation:
 *    First of all we must initialize our I2C module;
 *    Second, we have to connect to switcher: send slave address (I2C_SLAVE_ADDR_SWCH), then set necessary register;
 *    Third, after registers have been set, we must to run new XIicPs_MasterSendPolled(...), where we send 'Byte addr' and 'Data'(registers)
 *        to dspll (XIicPs_MasterSendPolled(arg1, arg2, arg3, I2C_SLAVE_ADDRESS_DSPLL)).
 */
...
int main(void) {
	...
	float start_freq;				// initial frequency ~56.352
	u8 freq = 4;
	...
	// Connect to 1 and 5 channel on switcher
	i2cAddr = CH_01_05;
	print("\n\r*****\tConnecting to switcher\t*****\n\r");
	Status = i2cConnect(&MyI2C, (u8*) &i2cAddr, 1);
	if (Status != XST_SUCCESS) {
